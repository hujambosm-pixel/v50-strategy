...
function buildPnlFloatCurve(allTrades, historicalCloses, contributions=[], opts={}) {
  ...
  const { floatOpenOnly=false, withContribs=false } = opts
  ...
  // withContribs: también añade fechas de aportaciones/retiradas al eje temporal
  if(withContribs) contributions.forEach(c=>{ if(c.type!=='dividendo'&&c.date&&(!firstTradeDate||c.date>=firstTradeDate)) dateSet.add(c.date) })
  const allDates=[...dateSet].sort()
  ...
  // withContribs: aportaciones/retiradas ordenadas cronológicamente para acumulación en runContrib
  const contribsSorted=withContribs?[...contributions].filter(c=>c.type!=='dividendo'&&c.date).sort((a,b)=>a.date.localeCompare(b.date)):[]
  const closed=allTrades.filter(t=>t.status==='closed').sort((a,b)=>(a.exit_date||'').localeCompare(b.exit_date||''))
  // Tres realizados en paralelo (mismo runDiv y mismo pnlFloat): principal (pnl_eur),
  // Sin FX (sin drift cambiario: precio×shares/fx_entry) y Sin Comm (pnl_eur+commission).
  let pnlClosed=0, pnlClosedSinFx=0, pnlClosedSinComm=0, closedIdx=0, runDiv=0, divIdx=0, runContrib=0, contribIdx=0
  const main=[], sinFx=[], sinComm=[]
  for(const date of allDates){
    while(divIdx<divsSorted.length&&divsSorted[divIdx].date<=date){ runDiv+=parseFloat(divsSorted[divIdx].amount||0); divIdx++ }
    if(withContribs) while(contribIdx<contribsSorted.length&&contribsSorted[contribIdx].date<=date){ const c=contribsSorted[contribIdx]; runContrib+=c.type==='retirada'?-parseFloat(c.amount||0):parseFloat(c.amount||0); contribIdx++ }
    while(closedIdx<closed.length&&(closed[closedIdx].exit_date||'')<=date){
      const ct=closed[closedIdx]; const cFx=parseFloat(ct.fx_entry||0)||1
      pnlClosed       += parseFloat(ct.pnl_eur||0)
      pnlClosedSinFx  += (parseFloat(ct.exit_price||0)-parseFloat(ct.entry_price||0))*parseFloat(ct.shares||0)/cFx
      pnlClosedSinComm+= parseFloat(ct.pnl_eur||0)+parseFloat(ct.commission||0)
      closedIdx++
    }
    let pnlFloat=0
    for(const t of allTrades){
    ...
    main.push({date,value:runContrib+runDiv+pnlClosed+pnlFloat})
    sinFx.push({date,value:parseFloat((runContrib+runDiv+pnlClosedSinFx+pnlFloat).toFixed(4))})
    sinComm.push({date,value:parseFloat((runContrib+runDiv+pnlClosedSinComm+pnlFloat).toFixed(4))})
  }
...
export default function Home() {
  ...
  const [calcPhase]=useState(0)  // siempre 0: quedó tras eliminar calcRankingFull (código muerto); WatchlistManager lo lee como prop
  ...
  const [candleH,setCandleH]=useState(480)     // candle chart height (bare/legacy; en normal usa fillHeight)
  ...
  const dashboardCurves = useMemo(()=>{
    ...
    // Patrimony curve (modo 'equity'): contributions + dividends + realizado + flotante día a día
    // Uses buildPnlFloatCurve(withContribs:true) with same openFloatCloses as P&L Nivel 1.
    // Robust _applyEnd_ replaces fragile today>lastW endpoint check.
    let curveWithContribs=[]
    if(contributions.length){
      const _cwcCloses=_pnlDailyReady?floatCloses:_openMode?openFloatCloses:null
      if(_cwcCloses&&Object.keys(_cwcCloses).length>0){
        curveWithContribs=buildPnlFloatCurve(tlTradesFiltered,_cwcCloses,contributions,{floatOpenOnly:!_pnlDailyReady,withContribs:true}).main
      } else {
        ...
        if(curveWithContribs.length===0&&openTrades.length){const anchor=cSorted[0]?.date||openTrades[0]?.entry_date||today;curveWithContribs=[{date:anchor,value:runContribW}]}
      }
      if(openTrades.length&&curveWithContribs.length){
        const _netContrib_=contributions.reduce((s,c)=>c.type==='aportacion'?s+parseFloat(c.amount||0):c.type==='retirada'?s-parseFloat(c.amount||0):s,0)
...
Si ocurre frecuentemente, reduce el texto pegado o actualiza tu plan en console.groq.com`)
  ...
  return (
    <>
      <Head>
        <title>Trading Simulator V9.822</title>
      ...
      <div className="app">
        ...
        <header className="header" style={{display:'flex',alignItems:'stretch',padding:0,height:TAB_H}} onContextMenu={e=>openCtx(e,'header')}>
          ...
          <div className="header-logo" onClick={()=>{setSidePanel('tradelog');setTlTab('dashboard')}} style={{display:'flex',alignItems:'center',padding:'0 16px',flexShrink:0,cursor:'pointer',position:'relative',zIndex:1000}}>
            <span className="dot"/>Trading Simulator V9.822
          </div>
              ...
              <div style={{display:'flex',flexDirection:'column',flex:1,overflow:'visible',minHeight:0}}>
                ...
                <div style={{overflowY:'auto',flex:1}}>
                  ...
                  {!wlLoading&&(()=>{
                    ...
                    const pendingSet=new Set((pendingOrders||[]).map(o=>(o.symbol||'').toUpperCase()).filter(Boolean))
                    const allFiltered=onlyOpen?all.filter(w=>w.symbol&&(openSymbols.has(w.symbol.toUpperCase())||pendingSet.has(w.symbol.toUpperCase()))):all
                    ...
                    return (<>{countBadge}{allFiltered.map((w,wIdx)=>{
                      ...
                      <div key={w.id||`${w.symbol}-${wIdx}`}
                        ...
                        {/* "X" borrar orden pendiente (directo, sin confirmación) — solo filas con pendiente */}
          ...
          <div className="content" style={{...(result?.isBareChart?{overflowY:'hidden'}:undefined),position:'relative'}}>
            ...
            <div style={{ position:'relative', height:'calc(100vh - 56px)', overflow:'hidden', zIndex:20 }}>
              <WatchlistManager
                ...
                onDisableAutoRefresh={()=>{ autoRefreshTriggered.current=true }}
              />
            </div>
            )}
            ...
            {mcResult&&sidePanel==='multi'&&(
              ...
              <div style={{flex:1,overflowY:'auto',padding:'0 0 20px 0'}}>
                ...
                <div className="equity-section" data-chart="equity"
                  ...
                  <div ref={mcHeaderRef} className="section-title"
                    ...
                    {mcPanelActivo?(<>
                      ...
                      {/* Leyenda de los indicadores. Va aquí y no en el gráfico porque
                          lightweight-charts flota el título de cada serie junto a su último punto, en
                          medio del área y encima de las velas. Aquí se lee de una vez y no tapa nada. */}
                      {mcIndicadoresActivo?.map(ind=>(
                        <span key={ind.name+ind.color} title={ind.name}
                          style={{display:'inline-flex',alignItems:'center',gap:4,alignSelf:'center',
                            fontFamily:MONO,fontSize:10,fontWeight:400,color:ind.color}}>
                          <span style={{width:10,height:2,borderRadius:1,background:ind.color,flexShrink:0}}/>
                          {ind.name||'—'}
                        </span>
                      ))}
                    ...
                    {mcPorActivo?(
                      ...
                      [
                        // Vista Simple retirada del multibacktest (V9.562): solo Compuesto. El botón rotula con
                        // mcSingleStratName —misma fuente que la tabla y la leyenda—, que prioriza la estrategia
                        // SELECCIONADA sobre la activa global. Este punto se quedó fuera de V9.710 y por eso el
                        // botón seguía mostrando la del selector superior aunque se hubiera ejecutado otra.
                        // La cascada posterior queda como red de seguridad: hoy es inalcanzable porque
                        // mcSingleStratName ya trae su propio fallback ('Estrategia activa').
                        {key:'compound',label:(mcSingleStratName||(mcDisplayResults[0]?.name||'').split(' · ')[0]||'Compuesto'),color:'#00e5a0',state:mcShowCompound,set:setMcShowCompound},
                ...
                {(()=>{
                  ...
                  // Sin activo seleccionado, este bloque seguía pintando TODAS las estrategias mientras
                  // los de al lado ya seguían a la activa. Mismo filtro que ellos, mismo estado.
                  ...
                  if(mcDisplayResults.length>1&&mcShowBHCompare&&mcResult.bhCurve?.length)
                    ...
                    mSeries=[...mSeries,{id:'__bh__',name:'B&H Diversif.',color:'#a0b4c8',compoundCurve:mcResult.bhCurve}]
                  // Con un activo seleccionado manda SU curva. assetCurves es contribución acumulada en
                  // euros y arranca en cero, así que se le suma el capital inicial para que el componente
                  // —que compara cada mes con el anterior y el primero con capitalIni— mida el mismo
                  // incremento mensual que mediría sobre una curva de patrimonio.
                  const _symSelM=mcActivoSel?.symbol
                  const _curvaAct=_symSelM
                    ?(mcHistSel.histResult?.assetCurves||[]).find(a=>a.symbol===_symSelM)?.data
                    :null
                  if(_symSelM&&_curvaAct?.length){
                    const serieAct=[{id:`activo__${_symSelM}`,name:_symSelM,
                      color:mcCurvaActivoSel?.[0]?.color||'#00d4ff',
                      compoundCurve:_curvaAct.map(p=>({date:p.date,value:capIniNum+p.value}))}]
                    return <div data-chart="monthly"><div style={{width:'calc(100% - 21px)',marginLeft:0}}>
                      <McMonthlyGainsChart series={serieAct} capitalIni={capIniNum} syncRef={chartSyncRef} axisWidth={mcAxisW}
                        soloEuros
                        nota={{texto:`solo ${_symSelM} · importes aproximados`,
                          detalle:'La curva por activo se dibuja sobre un eje reducido de ~400 fechas, así que el reparto por meses toma el primer y el último punto disponibles de cada mes y los importes no son exactos.'}}/>
                    </div></div>
                  }
                  if(!mSeries.some(s=>s.compoundCurve?.length)) return null
                ...
                {tlTab==='dashboard'&&(
                  <div style={{flex:1,display:'flex',flexDirection:'column',gap:0,minHeight:0,overflow:'hidden'}}>
                    {(()=>{
                      ...
                      const curveSinFx = Object.keys(sinFxByDate).sort().map(date=>({date,value:sinFxByDate[date]}))
                      const curveSinComm = Object.keys(sinCommByDate).sort().map(date=>({date,value:sinCommByDate[date]}))
                      // Flotante de HOY con precios live (liveFloatEur_), coherente con las tarjetas.
                      // Se aplica como endpoint más abajo (en la selección de eqDisp), igual en Nivel 1 y día a día.
                      const floatToday_ = openTrades.reduce((s,t)=>{const v=liveFloatEur_(t);return s+(v!=null?v:0)},0)
                      const openComm = openTrades.reduce((s,t)=>s+parseFloat(t.commission||0),0)
                      ...
                      let sfxC  = _dd ? _dd.sinFx   : curveSinFx
                      let scmC  = _dd ? _dd.sinComm : curveSinComm
                      // Endpoint LIVE de hoy (idéntico en Nivel 1 y día a día): último punto = realizado+dividendos+floatHoy = P&L TOTAL.
                      // Robusto ante last.date <,==,> today: solo ajusta la FECHA del último punto a hoy; el VALOR ya incluye
                      // el pnl_eur de las cerradas hoy (en cumPnl), no se pierde ningún realizado.
                      if(openTrades.length>0){
                        if(mainC.length===0){ const d0=openTrades[0].entry_date||today; mainC=[{date:d0,value:0}]; sfxC=[{date:d0,value:0}]; scmC=[{date:d0,value:0}] }
                        const _applyEnd_=(curve,val)=>{
                          if(!curve.length) return
                        ...
                        <div id="tlDashOuter" data-dash-outer="1" style={{display:'flex',flexDirection:'column',background:'var(--bg)',flex:1,minHeight:0,overflowY:'auto',overflowX:'hidden',alignItems:'stretch'}}>
                          ...
                          <div style={{display:'flex',alignItems:'center',gap:5,padding:'6px 12px',borderBottom:'1px solid var(--border)',background:'var(--bg2)',flexShrink:0,flexWrap:'nowrap',overflowX:'auto'}}>
                            ...
                            <button onClick={handleDashboardRefresh} disabled={tlRefreshing} title="Refrescar datos del Dashboard (Mercados, precios, flotantes, trades)"
                              style={{marginLeft:'auto',marginRight:124,display:'flex',alignItems:'center',gap:4,fontFamily:MONO,fontSize:11,padding:'2px 9px',borderRadius:10,border:'1px solid rgba(0,212,255,0.4)',background:'rgba(0,212,255,0.08)',color:'#00d4ff',cursor:tlRefreshing?'wait':'pointer',whiteSpace:'nowrap',flexShrink:0,opacity:tlRefreshing?0.6:1}}>
                          ...
                          <div style={{display:'flex',flex:1,minHeight:0,overflow:'hidden'}}>
                            ...
                            <div style={{flex:2.5,borderRight:'1px solid var(--border)',display:'flex',flexDirection:'column',overflow:'hidden',position:'relative',minWidth:0}}>
                              ...
                              <div style={{flex:2-tlEquityFlex,position:'relative',display:'flex',flexDirection:'column',overflow:'hidden',minHeight:0}}>
                                <div ref={tlInvestContainerRef} style={{flex:1,minHeight:0,height:'100%',position:'relative'}}>
