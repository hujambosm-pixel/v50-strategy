...
function calcMetrics(trades, capitalIni, capitalReinv, gananciaSimple, startDate, endDate) {
  ...
  const anios = startDate && endDate
    ? (new Date(endDate) - new Date(startDate)) / (365.25 * 86400000)
    : 1
  const safYears = Math.max(anios, 0.01)
  const capSimple = capitalIni + gananciaSimple
  const cagrS = Math.pow(capSimple / capitalIni, 1 / safYears) - 1
  ...
  return {
    ...
    ganSimple: gananciaSimple,
    ganComp: capitalReinv - capitalIni,
    cagrS: cagrS * 100, cagrC: cagrC * 100,
...
function CandleChart({ data, projR, projL, emaRPeriod, emaLPeriod }) {
  ...
  useEffect(() => {
    ...
    import('lightweight-charts').then(({ createChart, CrosshairMode, LineStyle }) => {
      ...
      const emaRSeries = chart.addLineSeries({ color: '#ffd166', lineWidth: 2, title: `EMA ${emaRPeriod}` })
      emaRSeries.setData(data.filter(d => d.emaR != null).map(d => ({ time: d.date, value: d.emaR })))
      const emaLSeries = chart.addLineSeries({ color: '#ff4d6d', lineWidth: 2, title: `EMA ${emaLPeriod}` })
      emaLSeries.setData(data.filter(d => d.emaL != null).map(d => ({ time: d.date, value: d.emaL })))
      if (projR?.length) {
        const projRS = chart.addLineSeries({ color: '#ffd166', lineWidth: 2, lineStyle: LineStyle.Dashed, title: '' })
        projRS.setData(projR.map(p => ({ time: p.date, value: p.value })))
      }
      if (projL?.length) {
        const projLS = chart.addLineSeries({ color: '#ff4d6d', lineWidth: 2, lineStyle: LineStyle.Dashed, title: '' })
        projLS.setData(projL.map(p => ({ time: p.date, value: p.value })))
      }
  ...
  return <div ref={containerRef} id="tv-chart" style={{ minHeight: 460 }} />
}
...
export default function Home() {
    ...
    try {
      const res = await fetch('/api/datos', {
        ...
        body: JSON.stringify({
          simbolo,
          cfg: {
            emaR: Number(emaR), emaL: Number(emaL),
            years: Number(years),
  ...
  const metrics = result
    ? calcMetrics(result.trades, capitalIni, result.capitalReinv, result.gananciaSimple,
  ...
  if (sp5 && tipoFiltro !== 'none') {
    const bloquea = tipoFiltro === 'precio_ema'
      ? sp5.precio < sp5.emaR
      : sp5.emaR < sp5.emaL
      ...
      <div className="app">
        <header className="header">
          ...
          <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text3)' }}>
            datos: Yahoo Finance · actualización diaria
          </div>
        </header>
        <div className="main">
          ...
          <div className="content">
            {sp5 && (
              <div className="sp500-bar">
                ...
                <span className={`val ${sp5.changePct >= 0 ? 'green' : 'red'}`} style={{fontSize:11}}>
                  {sp5.changePct >= 0 ? '▲' : '▼'} {fmt(Math.abs(sp5.changePct), 2)}%
                </span>
            ...
            {!loading && !result && !error && (
              <div className="empty-state">
                ...
                <div className="empty-desc">
                  Configura los parámetros en el panel izquierdo y pulsa <strong>▶ Ejecutar</strong> para ver el backtest y el gráfico interactivo.
                </div>
                <div style={{ marginTop: 16 }}>
                  <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text3)', marginBottom: 8 }}>SÍMBOLOS DE EJEMPLO</div>
                  <div className="ticker-grid">
                    {TICKERS.map(t => (
                      <div key={t} className="ticker-pill" onClick={() => setSimbolo(t)}>{t}</div>
                    ))}
                  </div>
                </div>
              </div>
              ...
              <>
                <div className="chart-wrap">
                  ...
                  <div style={{ display:'flex', gap:20, marginTop:10, fontFamily:'var(--mono)', fontSize:11, color:'var(--text3)' }}>
                    ...
                    <span><span style={{color:'#ffd166'}}>- -</span> Proyección EMA {emaR}</span>
                    <span><span style={{color:'#ff4d6d'}}>- -</span> Proyección EMA {emaL}</span>
                ...
                {result.trades?.length > 0 && (
                  <div className="equity-section">
                    <div className="section-title">Curva de Resultados por Operación</div>
                ...
                {result.trades?.length > 0 && (
                  <div className="trades-section">
                    <div className="section-title">Historial de Operaciones — {result.trades.length} trades</div>
                ...
                {!result.trades?.length && (
                  <div style={{ padding: '24px', fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--text3)' }}>
                    No se generaron operaciones en el periodo. Prueba a ampliar los años o ajustar los parámetros.
                  </div>
                )}
              </>
            )}
