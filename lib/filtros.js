...
const CAMPO_TICKER = { k: 'ticker', etiqueta: 'Ticker', tipo: 'texto', ancho: 60 }
...
export const FILTROS_CATALOGO = {
  indiceEma: {
    ...
    campos: {
      mercado: [CAMPO_TICKER, { k: 'periodo', etiqueta: 'EMA', tipo: 'numero', ancho: 50, min: 2, max: 500, fallback: 200 }],
      activo:  [              { k: 'periodo', etiqueta: 'EMA', tipo: 'numero', ancho: 50, min: 2, max: 500, fallback: 200 }],
    },
    prepara(f, ctx) {
  ...
  rsi: {
    ...
    // OJO con `min`: el renderizador convierte con `Number(x) || fallback` y 0 es falsy, así que
    // teclear 0 lo devuelve a 55. Con 1 se consigue lo mismo en la práctica.
    campos: (() => {
      const c = [
        { k: 'periodo', etiqueta: 'RSI', tipo: 'numero', ancho: 42, min: 2, max: 100, fallback: 14 },
        { k: 'min',     etiqueta: 'Mín', tipo: 'numero', ancho: 42, min: 0, max: 100, fallback: 55 },
        { k: 'max',     etiqueta: 'Máx', tipo: 'numero', ancho: 42, min: 0, max: 100, fallback: 100 },
      ]
