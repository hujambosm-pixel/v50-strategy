...
// ═══════════════════════════════════════════════════════════════════════════════
import { calcMACD, calcRSI } from './backtester'
...
// `label`, `params` y `campos` pueden ser el valor de siempre o un objeto indexado por ámbito
// ({mercado, activo}). Lo resuelve defDe(tipo, ambito), por el que pasan todos los consumidores.
// La diferencia real entre ámbitos es el TICKER: en mercado se configura; en activo no existe,
// porque el ticker es el propio símbolo. Por eso "benchmark" y no "índice": el ticker de mercado es
// configurable y puede ser un ETF u otra acción.
const CAMPO_TICKER = { k: 'ticker', etiqueta: 'Ticker', tipo: 'texto', ancho: 50 }

// ¿Hay que PROYECTAR la serie del filtro sobre las fechas del activo? Solo cuando las dos rejillas
// son distintas, es decir, cuando el filtro es semanal y el backtest NO. Ahí proyectar es
// obligatorio: sin la regla de la última semana cerrada se miraría al futuro, porque el cierre de la
// semana en curso no se conoce el martes.
// Si el backtest YA corre en semanal, la barra del día ES la vela semanal y su cierre ya se conoce
// al evaluarla —igual que en diario se usa el cierre del día—, así que proyectar solo retrasaría
// todo una vela sin ganar nada. Es la misma distinción que macd y rsi hacen con ctx.esSemanalNativo;
// aquí se factoriza porque indiceEma y cruceEma no la hacían y arrastraban ese retraso.
const proyectando = (f, ctx) => f.params?.intervalo === 'semanal' && !ctx.esSemanalNativo(f)

export const FILTROS_CATALOGO = {
  indiceEma: {
    label: { mercado: 'Benchmark > EMA', activo: 'Activo > EMA' },
    ambitos: ['mercado', 'activo'],
    intervaloConfigurable: true,
    params: {
      mercado: { ticker: '^GSPC', periodo: 10, intervalo: 'diario' },
      activo:  {                  periodo: 10, intervalo: 'diario' },
    },
    campos: {
      mercado: [CAMPO_TICKER, { k: 'periodo', etiqueta: 'EMA', tipo: 'numero', ancho: 40, min: 2, max: 500, fallback: 200 }],
      activo:  [              { k: 'periodo', etiqueta: 'EMA', tipo: 'numero', ancho: 40, min: 2, max: 500, fallback: 200 }],
    },
    prepara(f, ctx) {
      const src = ctx.serieDe(f)
      if (!src) return null
      // Proyectar SOLO cuando hay dos rejillas distintas: ver la nota de `proyectando`.
      const { closes, ema } = ctx.alineado(src, proyectando(f, ctx), Math.max(1, f.params?.periodo ?? 200))
      return i => { const c = closes?.[i], e = ema?.[i]; return c == null || e == null ? true : c >= e }
    },
  },
  cruceEma: {
    label: { mercado: 'Benchmark EMA (R>L)', activo: 'Activo EMA (R>L)' },
    ambitos: ['mercado', 'activo'],
    intervaloConfigurable: true,
    params: {
      mercado: { ticker: '^GSPC', periodoR: 10, periodoL: 11, intervalo: 'diario' },
      activo:  {                  periodoR: 10, periodoL: 11, intervalo: 'diario' },
    },
    campos: (() => {
      const per = [
        { k: 'periodoR', etiqueta: 'R', tipo: 'numero', ancho: 32, min: 2, max: 500, fallback: 10 },
        { k: 'periodoL', etiqueta: 'L', tipo: 'numero', ancho: 32, min: 2, max: 500, fallback: 11 },
      ]
      return { mercado: [CAMPO_TICKER, ...per], activo: [...per] }
    })(),
    prepara(f, ctx) {
      const src = ctx.serieDe(f)
      if (!src) return null
      const sem = proyectando(f, ctx)
  ...
  // En ámbito activo y semanal, con el backtest en diario, es una ventana de permiso lenta sobre
  // operativa rápida: la razón por la que se añadió.
  macd: {
    label: { mercado: 'MACD del benchmark (12/26/9)', activo: 'MACD del activo (12/26/9)' },
    ambitos: ['mercado', 'activo'],
    intervaloConfigurable: true,
    // 12/26/9 son fijos a propósito: son los valores canónicos y no hay campos que configurar.
    // En mercado sí hace falta el ticker: sin él, clavesAuxiliares no descargaría su serie.
    params: {
      mercado: { ticker: '^GSPC', intervalo: 'diario' },
      activo:  {                  intervalo: 'diario' },
    },
    campos: { mercado: [CAMPO_TICKER], activo: [] },
    prepara(f, ctx) {
      const src = ctx.serieDe(f)
      // null = la serie semanal del símbolo no se pudo descargar. El filtro no entra en el AND
      // (fail-open) y el símbolo viaja en avisosFiltros.sinSerieSemanal para que no sea silencioso.
      if (!src?.length) return null
      const prueba = (line, signal) => i => {
        const l = line?.[i], s = signal?.[i]
        return l == null || s == null ? true : l > s   // fail-open durante el calentamiento
      }
      const semanal = f.params?.intervalo === 'semanal'

      // Rama A — diario, o semanal cuando el backtest YA corre en semanal: en ambos casos las barras
      // están en la misma rejilla que assetDates, así que se calcula directo y NO se proyecta.
      // Proyectar aquí retrasaría todo una vela sin motivo (cf. esSemanalNativo).
      if (!semanal || ctx.esSemanalNativo(f)) {
        const { line, signal } = calcMACD(ctx.alineado(src, false, 1).closes, 12, 26, 9)
        return prueba(line, signal)
      }

      // Rama B — semanal sobre backtest diario. El MACD se calcula sobre los cierres semanales
      // CRUDOS: hacerlo sobre los ya proyectados sería una EMA de valores repetidos cinco veces, que
      // no es el MACD semanal. Línea y señal se proyectan después con la regla de la última semana
      // cerrada, la misma que usan los filtros de mercado.
      const sorted = [...src].sort((a, b) => a.date.localeCompare(b.date))
      const { line, signal } = calcMACD(sorted.map(d => d.close), 12, 26, 9)
      const wDates = sorted.map(d => d.date)
      return prueba(ctx.proyectarSemanal(line, wDates), ctx.proyectarSemanal(signal, wDates))
    },
  },
  rsi: {
    label: { mercado: 'RSI del benchmark', activo: 'RSI del activo' },
    ambitos: ['mercado', 'activo'],
    intervaloConfigurable: true,
    params: {
      mercado: { ticker: '^GSPC', periodo: 14, min: 55, max: 100, intervalo: 'diario' },
      activo:  {                  periodo: 14, min: 55, max: 100, intervalo: 'diario' },
    },
    // Si min > max el filtro no permite nada. Se deja pasar tal cual a propósito, sin intercambiar:
    // es un estado TRANSITORIO inevitable al teclear —para ir de 55-100 a 20-40 se escribe uno de los
    // dos primero—, y reordenar en ese instante haría los campos imposibles de editar. Intercambiar
    // en silencio además mentiría: se leería "mín 80, máx 20" y actuaría como 20-80.
    campos: (() => {
      const c = [
        { k: 'periodo', etiqueta: 'RSI', tipo: 'numero', ancho: 32, min: 2, max: 100, fallback: 14 },
        { k: 'min',     etiqueta: 'Mín', tipo: 'numero', ancho: 32, min: 0, max: 100, fallback: 55 },
        { k: 'max',     etiqueta: 'Máx', tipo: 'numero', ancho: 32, min: 0, max: 100, fallback: 100 },
      ]
      return { mercado: [CAMPO_TICKER, ...c], activo: [...c] }
    })(),
    prepara(f, ctx) {
      const src = ctx.serieDe(f)
      if (!src?.length) return null
      const periodo = Math.max(1, f.params?.periodo ?? 14)
      const lo = f.params?.min ?? 55, hi = f.params?.max ?? 100
      // Banda INCLUSIVA en los dos extremos. Fail-open mientras el RSI no tenga valor: son `periodo`
      // barras, y con el suavizado de Wilder tarda unos 3-4 periodos más en converger del todo.
      const prueba = (rsi) => i => { const v = rsi?.[i]; return v == null ? true : (v >= lo && v <= hi) }
      const semanal = f.params?.intervalo === 'semanal'

      // Rama A — diario, o semanal cuando el backtest YA corre en semanal: las barras están en la
      // misma rejilla que assetDates, así que se calcula directo y NO se proyecta.
      if (!semanal || ctx.esSemanalNativo(f)) {
        return prueba(calcRSI(ctx.alineado(src, false, 1).closes, periodo))
      }

      // Rama B — semanal sobre backtest diario. El RSI se calcula sobre los cierres semanales CRUDOS
      // —sobre los ya proyectados serían diferenciales cero cuatro de cada cinco barras— y se
      // proyecta después con la regla de la última semana cerrada.
      const sorted = [...src].sort((a, b) => a.date.localeCompare(b.date))
      return prueba(ctx.proyectarSemanal(calcRSI(sorted.map(d => d.close), periodo), sorted.map(d => d.date)))
    },
  },
}

// Vista de una entrada del catálogo YA resuelta para un ámbito: label, params y campos concretos.
// Todos los consumidores pasan por aquí, así que un tipo puede variar por ámbito sin que ninguno de
// ellos tenga que saberlo. Un valor que no sea objeto-por-ámbito se devuelve tal cual, de modo que
// un tipo futuro que no distinga ámbitos puede seguir declarando el valor a secas.
const porAmbito = (v, ambito, fallback) =>
  (v && typeof v === 'object' && !Array.isArray(v) && (AMBITOS.some(a => a in v))) ? (v[ambito] ?? fallback) : v
export function defDe(tipo, ambito) {
  const def = FILTROS_CATALOGO[tipo]
  if (!def) return null
  return {
    ...def,
    label:  porAmbito(def.label,  ambito, tipo),
    params: porAmbito(def.params, ambito, {}) || {},
    campos: porAmbito(def.campos, ambito, []) || [],
  }
}
...
export const nuevoFiltro = (tipo, ambito, activo = true) => ({
  ...
  ambito,
  activo,
  params: { ...defDe(tipo, ambito).params },
})
...
export function mergeFiltros(raw) {
  ...
  for (const f of raw) {
    ...
    vistos.add(id)
    // Params DEL ÁMBITO, y filtrados a sus claves conocidas: así un `ticker` guardado no queda
    // pegado a un filtro de ámbito activo —donde no significa nada— ni viaja al backend.
    const pDef = defDe(f.tipo, f.ambito).params
    const pGuardados = f.params && typeof f.params === 'object' ? f.params : {}
    const params = {}
    for (const k of Object.keys(pDef)) params[k] = k in pGuardados ? pGuardados[k] : pDef[k]
    out.push({ id, tipo: f.tipo, ambito: f.ambito, activo: !!f.activo, params })
  }
  return out
}
