...
export function evaluateExit(i, cfg, data, ind) {
...
export function runBacktestV50(data, sp500Data, cfg) {
  ...
  const trades       = []
  const triggerIns   = []  // DEBUG — entradas con filt=false
  const blockEvents  = {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (entradaPend) {
      ...
      if (triggerFires) {
        ...
        if (!filtroArr[i]) triggerIns.push({ fecha: bar.date, sp500C: +(_sp500Closes?.[i]?.toFixed(2)), spEmaR: +(_spEmaR?.[i]?.toFixed(2)), filt: filtroArr[i], tipoFiltro: cfg.tipoFiltro||cfg.filterType, px: +(prevBk?.toFixed(2)) })
  ...
  return { chartData, trades, capitalReinv, gananciaSimple, startDate, blockEvents, triggerIns }
}
