...
// ── evaluateFilter ───────────────────────────────────────────
// sp500Closes/spEmaR/spEmaL son arrays pre-calculados (performance)
export function evaluateFilter(i, cfg, sp500Closes, spEmaR, spEmaL) {
  ...
  if (tipoFiltro === 'ema_ema')   return spEmaR[i] < (spEmaL?.[i] ?? 0)
  return false
}
...
export function runBacktestV50(data, sp500Data, cfg) {
  ...
  const filtroArr = data.map((_, i) => {
    if (i === 0) return false
    return evaluateFilter(i, cfg, _sp500Closes, _spEmaR, _spEmaL)
  })
