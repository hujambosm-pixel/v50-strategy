  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (entradaPend) {
      ...
      if (triggerFires) {
        ...
        console.log(`[TRIGGER_IN backtester] fecha=${bar.date} sp500C=${_sp500Closes?.[i]?.toFixed(2)} spEmaR=${_spEmaR?.[i]?.toFixed(2)} filt=${filtroArr[i]} tipoFiltro=${cfg.tipoFiltro||cfg.filterType} px=${prevBk?.toFixed(2)}`)
