...
// Métricas crudas: {winRate, cagr, cagrRobust, maxDD}. cagrRobust NO se persiste en ranking_results
// → fuera de una corrida se usa el mismo fallback de siempre: cagrRobust ?? cagr.
...
export function scoreHistoricoDe(m,f,w){
  ...
  return Math.max(0,Math.min(100,
    normDynVal(m.winRate,            f.wrFl,f.wrCe)*w.wrPct+
    normDynVal(m.cagr,               f.caFl,f.caCe)*w.cagrPct+
    normDynVal(m.cagrRobust??m.cagr, f.crFl,f.crCe)*w.cagrRobPct-
    normDynVal(m.maxDD,              f.ddFl,f.ddCe)*w.ddPct))
}
