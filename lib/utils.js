...
// Métricas crudas: {winRate, cagr, robustez, maxDD}. La robustez (0-100: qué parte de las ganancias
// NO depende del mejor trade) se persiste en ranking_results.robustez_pct (sql/add_robustez_pct.sql);
// si una fila aún no la tiene, ese componente se marca como no disponible y aporta 0 puntos.
...
// Umbrales ABSOLUTOS de normalización, leídos de settings (antes se calculaban por percentiles del
// universo, lo que hacía que el score de un activo dependiera de con quién se comparara).
// suelo → 0 puntos, techo → 100 puntos, en medio proporcional, fuera de rango clampado.
// Max drawdown va INVERTIDO (suelo = peor, techo = mejor): normDynVal soporta floor>ceil de forma
// natural, así que menos drawdown da más puntos sin código especial.
// Mantiene la MISMA forma de objeto que la antigua floorsDe para no romper a los llamadores.
export function umbralesDe(sett){
  const r = sett?.ranking || {}
  return {
    wrFl: r.rankingWinRateFloor  ?? 25, wrCe: r.rankingWinRateCeil  ?? 65,
    caFl: r.rankingCAGRFloor     ??  0, caCe: r.rankingCAGRCeil     ?? 40,
    crFl: r.rankingRobustezFloor ?? 30, crCe: r.rankingRobustezCeil ?? 85,
    ddFl: r.rankingMaxDDFloor    ?? 50, ddCe: r.rankingMaxDDCeil    ?? 10,
  }
}
...
export function desgloseScoreHistorico(m,f,w){
  ...
  // Sin robustez (fila aún no recalculada) → componente NO DISPONIBLE: aporta 0, no se inventa nada
  const robustez = m.robustez!=null
    ? comp(m.robustez, f.crFl,f.crCe,w.cagrRobPct)
    : {valor:null,norm:null,peso:w.cagrRobPct,puntos:0,noDisponible:true}
  // Max drawdown: la INVERSIÓN vive en los umbrales (suelo=peor, techo=mejor), así que menos
  // drawdown ya da más puntos y el componente SUMA como los demás. Restar aquí, con esos umbrales,
  // penalizaría justo a los activos de menor drawdown.
  const maxDD    = comp(m.maxDD,   f.ddFl,f.ddCe,w.ddPct)
  const total=Math.max(0,Math.min(100,
    winRate.puntos+cagr.puntos+robustez.puntos+maxDD.puntos))
  return {winRate,cagr,robustez,maxDD,total,robustezNoDisponible:robustez.noDisponible===true}
}
