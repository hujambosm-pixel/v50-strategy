...
// -99 es el CENTINELA de "CAGR no calculable": se excluye de los percentiles de cagr
// porque no es una rentabilidad real y hundía el suelo, clampando a 100 cualquier valor verdadero.
// Si tras filtrar un array queda vacío, pctOf devuelve 0 → suelo=techo → normDynVal da 50 (neutro).
const _esCentinela = (v)=>v==null||v<=-99
export function floorsDe(metricas,pct){
  const wr=[],ca=[],cr=[],dd=[]
  ;(metricas||[]).forEach(m=>{
    if(!m) return
    if(m.winRate!=null) wr.push(m.winRate)
    if(!_esCentinela(m.cagr)) ca.push(m.cagr)
    // Robustez: 0-100 por construcción, sin centinelas ni fallback. Sin dato → fuera del universo.
    if(m.robustez!=null) cr.push(m.robustez)
