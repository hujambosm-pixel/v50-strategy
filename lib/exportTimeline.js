...
// exportGantt     : diagrama mensual 3 filas/activo (vista Gantt)
...
// ── Helpers de formato ────────────────────────────────────────────────────────
// 1 decimal, coma decimal, con signo ("+3,2%" / "-3,7%")
function fmtPct(v) {
  if (v == null || isNaN(v)) return '-'
  const sign = v >= 0 ? '+' : ''
  return sign + Number(v).toFixed(1).replace('.', ',') + '%'
}
// 2 decimales, coma decimal, con signo ("+28,14%" / "-3,72%")
function fmtPct2(v) {
  if (v == null || isNaN(v)) return '-'
  const sign = v >= 0 ? '+' : ''
...
// isOngoing=true → versión oscura (posición en curso); false → vivido (cierre)
function bgColor(pct, { isOngoing = false } = {}) {
  if (isOngoing) {
...
// P&L(%): string con símbolo % ("+28,14%"). P&L(€): número puro sin € ni +.
export function exportHistorial({ mcResult, mcSelected, baseCfg, stratName = 'estrategia' }) {
  ...
  allTrades.forEach((t, i) => {
    ...
    setC(ws, row, 6, mkCell(fmtPct2(pnlPct),                                  bg, pnlFg,    true,  9, 'right'))
  ...
  const TOT_BG = '1a0d2e'
  ;['TOTAL', '', '', '', '', '', fmtPct2(avgPct), n2(sumPnlEur), sumDias, ''].forEach((v, c) => {
...
// ══════════════════════════════════════════════════════════════════════════════
// EXPORTAR GANTT — diagrama mensual 3 filas por activo (vista Gantt)
// ══════════════════════════════════════════════════════════════════════════════
// Estructura:
//   Fila 0 (header): años merged, fondo #071929
//   Fila 1 (header): meses (Ene…Dic), fondo #0f2942
//   Filas 2+ (datos): 3 filas por activo
//     Fila base+0 : Ops (▲ entrada, ▼ cierre, X descartado)
//     Fila base+1 : P&L% string con símbolo (solo si hay cierres)
//     Fila base+2 : P&L€ número puro (solo si hay cierres)
// Columnas fijas A-D merged sobre las 3 filas: Activo | Ops | Cap.inv.(€) | P&L€Tot
export function exportGantt({
  ...
  // Colores de cabecera
  const HDR_YR  = '071929'  // fondo fila años
  const HDR_MO  = '0f2942'  // fondo fila meses
  const HDR_FG  = 'ffffff'
  const FIX_A_BG = '1e2d45'  // cols A-B (Activo, Ops)
  const FIX_C_BG = '1a3a5c'  // cols C-D (Cap.inv., P&L€)

  // ── Cabecera fila 0 cols 0-3: etiquetas fijas, merged con fila 1 ────────────
  setC(ws, 0, 0, mkCell('Activo',       HDR_YR, HDR_FG, true, 10, 'left'))
  setC(ws, 0, 1, mkCell('Ops',          HDR_YR, HDR_FG, true,  9, 'center'))
  setC(ws, 0, 2, mkCell('Cap.inv. (€)', HDR_YR, HDR_FG, true,  9, 'right'))
  setC(ws, 0, 3, mkCell('P&L€ Tot',     HDR_YR, HDR_FG, true,  9, 'right'))
  for (let c = 0; c < FIXED; c++) setC(ws, 1, c, mkCell('', HDR_MO))
  mgs.push({ s:{r:0,c:0}, e:{r:1,c:FIXED-1} })

  // ── Cabecera columnas de mes: fila 0 = años (merged), fila 1 = meses ────────
  ...
  months.forEach((m, mi) => {
    ...
    if (m.year !== prevYear) {
      ...
      setC(ws, 0, col, mkCell(String(m.year), HDR_YR, HDR_FG, true, 11))
      yearStartCol = col
      prevYear = m.year
    } else {
      setC(ws, 0, col, mkCell('', HDR_YR))
    }
    setC(ws, 1, col, mkCell(MONTHS_ES[m.month], HDR_MO, HDR_FG, false, 9))
  })
  if (prevYear !== null) {
  ...
  symbols.forEach((sym, si) => {
    ...
    // Col A: Activo
    setC(ws, base,   0, mkCell(sym, FIX_A_BG, 'ffffff', true,  9, 'left'))
    setC(ws, base+1, 0, mkCell('',  FIX_A_BG))
    setC(ws, base+2, 0, mkCell('',  FIX_A_BG))
    // Col B: Ops
    setC(ws, base,   1, mkCell(totalOps || '-', FIX_A_BG, 'ffffff', false, 9, 'center'))
    setC(ws, base+1, 1, mkCell('',              FIX_A_BG))
    setC(ws, base+2, 1, mkCell('',              FIX_A_BG))
    // Col C: Cap.inv.(€)
    setC(ws, base,   2, mkCell(n2(totalCapInv), FIX_C_BG, 'c8dff5', false, 9, 'right'))
    setC(ws, base+1, 2, mkCell('',              FIX_C_BG))
    setC(ws, base+2, 2, mkCell('',              FIX_C_BG))
    // Col D: P&L€ Tot
    setC(ws, base,   3, mkCell(n2(totalEur),    FIX_C_BG, pnlFg, true, 9, 'right'))
    setC(ws, base+1, 3, mkCell('',              FIX_C_BG))
    setC(ws, base+2, 3, mkCell('',              FIX_C_BG))

    // Merge columnas fijas sobre las 3 filas
    for (let c = 0; c < FIXED; c++)
      mgs.push({ s:{r:base,c}, e:{r:base+2,c} })
    ...
    months.forEach((m, mi) => {
      ...
      // Cierres del mes (excluye virtual close)
      ...
      // Posiciones en curso (ni entran ni salen este mes)
      const ongoing = symTrades.filter(t =>
        t.entryDate < mStart && (!t.exitDate || t.exitDate >= mNext) &&
        !inMonth(t.entryDate, m.year, m.month) && !inMonth(t.exitDate, m.year, m.month))
      // Señales descartadas este mes
      const discThisMo = symDisc.filter(t => inMonth(t.entryDate, m.year, m.month))

      // Color de fondo (las 3 filas comparten el mismo color)
      ...
      if (hasClosed) {
        ...
        avgPct = ongoing.reduce((s, t) => s + (t.pnlPct || 0), 0) / ongoing.length
      ...
      setC(ws, base,   col, mkCell(opsFinal, bg, opsFinal ? 'e2e8f0' : textFg, opsFinal.length > 0, 9))
      setC(ws, base+1, col, mkCell(pctText,  bg, textFg, false, 8))
      setC(ws, base+2, col, mkCell(monthEur, bg, textFg, false, 8))
    })
  })

  ws['!ref']    = XLSX.utils.encode_range({ s:{r:0,c:0}, e:{r:2+symbols.length*3-1, c:totalCols-1} })
  ws['!merges'] = mgs
  ws['!cols']   = [
  ...
  ws['!rows']   = [
    { hpt:18 }, { hpt:14 },
    ...symbols.flatMap(() => [{ hpt:14 }, { hpt:14 }, { hpt:14 }]),
  ]
  ws['!views']  = [{
...
// ══════════════════════════════════════════════════════════════════════════════
// OJO — hoy nadie llama a esta función: handleExport despacha exportGantt o exportHistorial. Si se
// reactiva, `filtros` debe llegar ya APLANADO (aplanarFiltros de lib/filtros), no como la lista del
// estado: el backend solo entiende la forma plana y, si recibe la lista, no reconocería nada y
// dejaría de filtrar en silencio.
