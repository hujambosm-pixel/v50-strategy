...
// ── Colores Gantt (sin '#') — misma escala que GanttChart.barColor() ──────────
...
// Versión pastel para trades descartados
...
// mcResult: resultado del backtest; mcSelected: activos; baseCfg: para filename
// discardedTrades: array de trades descartados (ya cargados desde el Gantt) o null
export function exportGantt({
  ...
  // Fila 0 (fijo, cols 0-3): headers fijos
  setC(ws, 0, 0, mkCell('Activo',   HDR_BG, HDR_FG, true, 10, 'left'))
  setC(ws, 0, 1, mkCell('Ops',      HDR_BG, HDR_FG, true,  9))
  setC(ws, 0, 2, mkCell('Win%',     HDR_BG, HDR_FG, true,  9))
  setC(ws, 0, 3, mkCell('P&L€ Tot', HDR_BG, HDR_FG, true,  9))

  // Fila 1 (fijo, cols 0-3): blank, merge con fila 0
  ...
  symbols.forEach((sym, si) => {
    ...
    setC(ws, row, 1, mkCell(totalOps || '-',                        FIX_BG, 'e2e8f0'))
    setC(ws, row, 2, mkCell(totalOps > 0 ? fmtPct(winPct) : '-',  FIX_BG, winFg, true))
    setC(ws, row, 3, mkCell(totalOps > 0 ? fmtEur(totalEur) : '-', FIX_BG, pnlFg, true, 9, 'right'))
    ...
    weeks.forEach((w, wi) => {
      ...
      if (activeExec.length > 0) {
        ...
        const text   = isExit ? String(Math.round(Math.abs(t.pnlPct || 0))) + '%' : ''
        setC(ws, row, col, mkCell(text, bg, 'ffffff', !!text, 8))
      } else if (activeDisc.length > 0) {
        const t      = activeDisc[0]
        const bg     = ganttColorLight(t.pnlPct)
        const isExit = t.exitDate && t.exitDate >= wStart && t.exitDate < wEnd
        const text   = isExit ? String(Math.round(Math.abs(t.pnlPct || 0))) + '%' : ''
        setC(ws, row, col, mkCell(text, bg, '4a5568', false, 8))
      } else {
